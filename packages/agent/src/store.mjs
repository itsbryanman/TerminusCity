...
export class EventStore {
  ...
  async append(event) { const ephemeral = event.persist === false; const next = ephemeral ? this.sequence : this.sequence + 1; const stored = validateEvent({ ...event, ...(ephemeral ? {} : { seq: next }) }); if (!ephemeral) { this.sequence = next; await appendFile(this.eventsFile, `${JSON.stringify(stored)}\n`, { mode: 0o600 }); await chmod(this.eventsFile, 0o600).catch(() => {}); } this.state = reduceCity(this.state, stored); if (!ephemeral) this._schedulePersist(); return stored; }
  ...
  async _persist() { if (!this.state) return; const state = structuredClone(this.state); state.metrics = { cpuPct: 0, memoryPct: 0, history: [] }; state.network = { rxBytes: 0, txBytes: 0 }; const snapshot = { lastSequence: this.sequence, state }; const tmp = `${this.stateFile}.tmp`; await writeFile(tmp, JSON.stringify(snapshot), { mode: 0o600 }); await rename(tmp, this.stateFile); }
